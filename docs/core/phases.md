...
own files; "add a syscall" is 2 files, down from 6); 2b done (pointer
arguments are declared in the row and checked by the dispatcher; `hw_user_range_ok`
has one call site; error precedence moved - see core_c4_abi.md).** Stage 1 is the vocabulary, the
...
**In progress (2026-09-23): steps 1 and 2 of 4 done.** A driver is a descriptor in its
...
**input device 4 -> 1**; virtio-net moved, **network interface 4 -> 1**. Building it found that no boot had ever exercised the
keyboard's interrupt path - nobody types in CI - so the keyboard's probe now makes
the controller raise IRQ1 and the gate asserts it arrived. Next: block and
filesystems, then the display. See `implementation_progress/core_c7_devices.md`.
