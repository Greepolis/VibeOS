...
| Program Loading | **Rewrite underway** ([docs/exec/](exec/README.md)). **X-P0 done**: every exec refusal names itself. Eleven reasons with a counter each, one log line per refusal carrying the path and the step, an `exec` console command, and a boot gate that asserts *which* reasons appear rather than a total - so one `no-memory` hidden among forty expected `not-found`s is caught. **X-P1 done**: the segment layout already lived in portable code, so the plan's second parser was not written - what was missing was that none of it was reachable except through a boot. Twelve host cases against a hand-built ELF64, including the two auxv rules that were comments. **X-P2 attempted and turned off**: mapping read-only image pages from the page cache removes 91% of the copying (4067 of 4479 pages) and took 8 boots from a 22/24-with-no-wedge parent to 6/8 with two wedges. Measured twice. The copy-on-write defect it was exposing is now **fixed**, and that removed the wedges entirely - but it stays disabled behind `if (0)`: 21/24 against 23/24, and all three failures were segfaults in Linux binaries at near-null addresses in a different program each time, this project's oldest corruption signature. Narrowed suspicion recorded: the branch creates a second source of truth for which file is being loaded | [exec.md](implementation_progress/exec.md) |
