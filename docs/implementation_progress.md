...
| Tasks and Scheduling | **Rewrite underway** ([docs/sched/](sched/README.md)): S-P0 to S-P4 done, **S-P5 built and tested but not yet wired**. A transition table that refuses illegal state changes, tenancy on every task reference, a tested run queue, lifetime, accounting charged to exactly one place per tick, and a policy layer with classes, nice-weighted fairness and affinity - measured over tens of thousands of ticks rather than asserted by inspection, and every rule confirmed by breaking it. Next: wire the picker, then work stealing | [sched.md](implementation_progress/sched.md) |
