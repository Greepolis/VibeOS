...
| Splitting arch_hw.c | **First cut done**: 9088 to 8544 lines. The Linux socket syscalls moved to their own file behind a declared seam (arch_hw_internal.h). The file had grown 633 lines during a session spent reducing it, so this plan carries the line count as a completion condition rather than a side effect | [arch_extraction.md](implementation_progress/arch_extraction.md) |
