...
| Memory Manager | **Rewrite underway** (ADR-0007), P0, P1 and P2 done. Physical frames have one owner in `kernel/mm/frame.c`; address spaces record what they own in the page-table entry (`kernel/mm/vmspace.c`), so teardown, fork, munmap and the copy-on-write fault no longer infer ownership from permission bits - the cause of the defect that survived four fixes. No page-table write outside `kernel/mm/`, checked on every build. Next: P3, regions. Plan in [docs/mm/](mm/README.md); current state in [memory_manager.md](implementation_progress/memory_manager.md) | [memory_manager.md](implementation_progress/memory_manager.md) |
