...
| Program Loading | **Rewrite underway** ([docs/exec/](exec/README.md)). **X-P0 done**: every exec refusal names itself. Eleven reasons with a counter each, one log line per refusal carrying the path and the step, an `exec` console command, and a boot gate that asserts *which* reasons appear rather than a total - so one `no-memory` hidden among forty expected `not-found`s is caught. Next: X-P1, the parse comes out of the architecture layer | [exec.md](implementation_progress/exec.md) |
