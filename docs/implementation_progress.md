...
| Tasks and Scheduling | **Rewrite underway** ([docs/sched/](sched/README.md)): S-P0 to S-P4 done - counters and a 	asks view, a transition table that refuses illegal state changes, tenancy on every task reference, a run queue that is a tested data structure, lifetime, and now **accounting**: every tick charged to exactly one place, asserted as an identity rather than a tolerance, with a separate counter for refused ticks because a refused tick keeps the identity intact. Next: S-P5, policy - priorities, fairness, affinity | [sched.md](implementation_progress/sched.md) |
