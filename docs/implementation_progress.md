...
| Memory Manager | **Rewrite underway** (ADR-0007). **P0-P3 done; P4 at steps 1-2 of 3 with image pages mapped from the cache; P6 step 1 (the reverse map) built and tested, not yet wired**. Physical frames have one owner in `kernel/mm/frame.c`; address spaces record what they own in the page-table entry (`kernel/mm/vmspace.c`), so nothing infers ownership from permission bits any more. No page-table write outside `kernel/mm/`, checked on every build. Host tests, 150 torture seeds and single boots are green; the repeat-boot criterion is not, and closing that comes before P3. Plan in [docs/mm/](mm/README.md); current state in [memory_manager.md](implementation_progress/memory_manager.md) | [memory_manager.md](implementation_progress/memory_manager.md)  [cow_stale_page.md](implementation_progress/cow_stale_page.md)  [mm_rmap.md](implementation_progress/mm_rmap.md) |
