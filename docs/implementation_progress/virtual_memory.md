...
Status: In Progress (hardware paging and per-process isolation verified)
...
## Pending
- Route `kernel/mm/vm.c` map/unmap/protect through the runtime page-table implementation so the portable model becomes the live authority.
...
## Next checkpoint
- Unify the portable VM API and runtime page-table backend, then drive demand-paging and protection-fault tests in QEMU.
