/* Moved out of kernel/fs/ in I5c.
 *
 * It lived there, and that said it was a filesystem concern. It is not: ext2
 * wants a journal, FAT has none, and the guarantee - "these writes happen
 * together or not at all" - has nothing to do with what a directory looks
 * like. A layer named after one of its customers is a layer the next customer
 * has to be talked into using.
 *
 * The second customer has not arrived yet, and the honest note is that the one
 * the plan expected turned out not to want it: writing a GPT was considered
 * and rejected, because a GPT already carries two CRC-checked copies of its own
 * table and a journal underneath would be a second recovery mechanism that has
 * to agree with the first. See docs/implementation_progress/io_gpt.md. What
 * this is for is updates with no scheme of their own.
 */
