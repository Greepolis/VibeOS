...
- `qemu-smoke-ovmf-linux.sh`: Linux OVMF smoke gate that boots from EFI media layout (`artifacts/efi_root`), validates `BOOTX64.EFI` as PE32+ EFI app, and asserts `BOOT_OK`.
- `run-qemu-ovmf.ps1`: Windows OVMF smoke runner for EFI media validation with structured summary output (`pass`, `timeout`, `fail`, `infra_error`).
