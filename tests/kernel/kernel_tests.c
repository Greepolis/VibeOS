...
static int test_kmain(void) {
    ...
    /* The boot event was signalled by vibeos_user_signal_boot_event, which
     * went with the dispatcher it called. Asserting it is signalled now
     * would be asserting the effect of a function that does not exist. */
...
/* test_syscalls went with kernel/core/syscall.c.
 * It was 687 lines, 263 of its references were to the dispatcher and 22 to
 * anything else - so it was a test of a switch statement no process could
 * reach, and its greenness was part of why this suite always passed. The
 * live syscall path is the arch layer's Linux table; C4 is where it gets a
 * vocabulary and a test worth having. */
...
static int test_user_api_and_bootloader(void) {
    ...
    /* All four report 0 now: the functions behind them went with the
     * dispatcher they called in-process. Asserted rather than deleted,
     * because a capability flag that outlives its implementation is a claim
     * nothing backs, and this assertion is what noticed. */
    ...
    /* The security-label and boot-event calls went with the half of
     * user_api.c that reached the portable kernel by calling
     * vibeos_syscall_dispatch in-process - which is not how a syscall works
     * and not a path any ring-3 program took. What is left here is the
     * bootloader and handle coverage, which is live. */
...
static int test_abi_vocabulary_and_linux(void) {
    ...
    for (id = 1; id < (uint32_t)VIBEOS_OP_COUNT; id++) {
        ...
        if ((e->checks & ~(VIBEOS_CHECK_USER_MEMORY | VIBEOS_CHECK_SIGNAL_PERMIT |
            ...
            return -1;   /* a check that names nothing the kernel has */
        }
        for (k = id + 1u; k < (uint32_t)VIBEOS_OP_COUNT; k++) {
            if (strcmp(e->name, vibeos_op_entry((vibeos_op_id_t)k)->name) == 0) {
                printf("abi: name %s declared twice\n", e->name);
                return -1;
            }
        }
    }
    ...
    {
        static const vibeos_row_t good[] = {
            { 7, "seven", VIBEOS_OP_READ, abi_test_handler },
            { 700, "high", VIBEOS_OP_WRITE, abi_test_handler },
        };
        static const vibeos_row_t twice_inside[] = {
            { 8, "a", VIBEOS_OP_OPEN, abi_test_handler },
            { 8, "b", VIBEOS_OP_CLOSE, abi_test_handler },
        };
        static const vibeos_row_t twice_across[] = {
            { 7, "seven-again", VIBEOS_OP_OPEN, abi_test_handler },
        };
        static const vibeos_row_t no_handler[] = { { 9, "nine", VIBEOS_OP_OPEN, 0 } };
        static const vibeos_row_t no_op[] = { { 10, "ten", VIBEOS_OP_NONE, abi_test_handler } };
        vibeos_call_t call;
        ...
        if (vibeos_abi_linux_register(no_op, 1u) == 0) { return -1; }
        {
            ...
            static vibeos_row_t bad_arg[1] = { { 11, "eleven", VIBEOS_OP_READ, abi_test_handler } };
            static vibeos_row_t bad_len[1] = { { 12, "twelve", VIBEOS_OP_READ, abi_test_handler } };
            static vibeos_row_t ok_ptr[1] = { { 13, "thirteen", VIBEOS_OP_READ, abi_test_handler } };
